...
class MovieSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        # count = linecache.getline("MovieList.txt", 1)
        # print(count)
        #lines = count.split('\n')
        for line in lines:
            if (line != '') and (not line in self.s):
                #if  not os.path.exists('data/%s' % line.split('/')[-2]):
                self.start_urls.append(line)


        # for allLines in fileinput.input("todolist.txt"):
        #     # allLines = f.read()
        #     lines = allLines.split('\n')
        #     for line in lines:
        #         if (line != '') and (not line in self.s):
    ...
    def parse_review_list(self, response):
        ...
        if next == None:
            ...
            f.write(response.url.split('reviews')[0] + '\n')
            f.close()
        else:
            yield scrapy.Request(response.url.split('?')[0] + next, self.parse_review_list)
            # if next == None:
            #     f = open('doneList.txt', 'a')
